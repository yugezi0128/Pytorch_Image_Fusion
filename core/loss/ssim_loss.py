...
class SSIM_Loss(nn.Module):
	...
	def __init__(self, num_channels=3, C=9e-4, device='cuda:0'):
	...
	def forward(self, input_images, output_images):
		batch_size, num_channels = input_images.shape[0], input_images.shape[1]
		ssim_loss = 0
		for batch in range(batch_size):
			input_image, output_image = input_images[batch], output_images[batch]
			...
			input_output_var = torch.mean(input_image * output_image, dim=[1, 2]) - input_image_mean * output_image_mean
